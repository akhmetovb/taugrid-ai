...
### 2D/3D Twin View Generation

- Input: current canvas graph (nodes, edges, coordinates) and project context.
- Execution: rendered live, client-side, from current canvas state — no background task required.
- Output: interactive 2D/3D Twin View panel using Mapbox GL JS, updating in real time as the layout changes.
